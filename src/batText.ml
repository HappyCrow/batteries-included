...
(* return all starting indexes of x in s in the reverse order;
   i.e. the rightmost occurence of x in s is given first *)
let rev_find_all s x =
  let x_len = length x in
  let rec loop acc offset =
    try
      let i = find_from s offset x in
      raise (Found i)
    with
    | Found i -> loop (i :: acc) (offset + x_len)
    | Not_found
    | Out_of_bounds -> acc
  in
  loop [] 0
